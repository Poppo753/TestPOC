...
/** How it works: five spatially distinct stations and one selected capital unit. */
export function createScene(THREE, scene, quality) {
  ...
  const colors = [COLORS.mint, COLORS.amber, COLORS.violet, COLORS.cyan, COLORS.mint];
  const stations = xs.map((x, index) => {
    ...
    if (index === 4) geometry = new THREE.IcosahedronGeometry(.47, 1);
  ...
  const route = createTube(THREE, resources, [[-3.4, -.25,.12],[-2.55,.05,.25],[-1.7,-.25,-.35],[-.85,.1,0],[0,-.16,.12],[.85,.05,.25],[1.7,-.18,-.1],[2.55,.05,.2],[3.4,-.2,0]], COLORS.cyan, .022); route.mesh.material.opacity = .36; route.mesh.material.transparent = true; root.add(route.mesh);
  ...
  function update(time, delta) {
    ...
    capital.position.copy(route.curve.getPoint(state.current / 4)); capital.position.y += .13;
