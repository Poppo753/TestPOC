...
export class SceneController {
  constructor(THREE, host, canvas, config, quality, sceneName, factory) {
    ...
    host.addEventListener('pointermove',this.onPointer,{passive:true}); document.addEventListener('visibilitychange',this.onVisibility);
  ...
  render(time,delta){this.world.update(time,delta);const motion=this.quality.reducedMotion?0:1;this.camera.position.x+=(this.baseCamera.x+this.pointer.x*.16*motion-this.camera.position.x)*.025;this.camera.position.y+=(this.baseCamera.y-this.pointer.y*.1*motion-this.camera.position.y)*.025;this.camera.lookAt(0,-.1,0);this.renderer.render(this.scene,this.camera);this.debug.update(time);}
  ...
  dispose(){this.stop();this.host.removeEventListener('pointermove',this.onPointer);document.removeEventListener('visibilitychange',this.onVisibility);this.resizeObserver.disconnect();this.intersectionObserver.disconnect();this.debug.dispose();this.world.dispose();this.renderer.dispose();this.renderer.forceContextLoss?.();}
}
