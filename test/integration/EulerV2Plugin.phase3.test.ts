...
describe("EulerV2Plugin - FASE 3: Leverage Atomico", function () {
    ...
    before(async function () {
        ...
        await (await beacon.updateImplementation("EulerVaultRegistry", await eulerVaultRegistry.getAddress())).wait();
        ...
        console.log("   ✅ EulerVaultRegistry deployed and configured");
    ...
    describe("1. Verify Base Components Work", function () {
        ...
        it("Should enable collateral and controller", async function () {
            ...
            await (await eulerPlugin.enableCollateral(ADDRESSES.WETH_VAULT)).wait();
            await (await eulerPlugin.enableController(ADDRESSES.USDC_VAULT)).wait();
            
            expect(await evc.isCollateralEnabled(pluginAddress, ADDRESSES.WETH_VAULT)).to.be.true;
            expect(await evc.isControllerEnabled(pluginAddress, ADDRESSES.USDC_VAULT)).to.be.true;
    ...
    describe("2. Open Leverage Position (Atomic)", function () {
        ...
        it("Should open leverage position with 1inch swap", async function () {
            const pluginAddress = await leveragePlugin.getAddress();
            const pluginWeth = await weth.balanceOf(pluginAddress);
            ...
            // Parameters
            const collateralAmount = pluginWeth; // Use all WETH as collateral
            const borrowAmount = ethers.parseUnits("10", 6); // Borrow 10 USDC
            ...
            try {
                const swapResult = await get1inchSwapData(
                    ADDRESSES.USDC,             // From: USDC (borrowed)
                    ADDRESSES.WETH,             // To: WETH (add to collateral)
                    borrowAmount.toString(),
                    ADDRESSES.SWAPPER,          // Swapper will have the USDC
                    ADDRESSES.WETH_VAULT,       // Send WETH to vault for skim
                    1                            // 1% slippage
                );
                ...
                // Build swapData for Euler Swapper
                const swapData = buildSwapperCalldata(
                    swapResult.tx.to,
                    swapResult.tx.data,
                    ADDRESSES.USDC,
                    ADDRESSES.WETH,
                ...
                const minCollateralReceived = BigInt(swapResult.dstAmount) * 95n / 100n; // 5% slippage
                const deadline = Math.floor(Date.now() / 1000) + 3600;
                
                // Build OpenLeverageParams
                const params = {
                ...
                console.log("\n🚀 Executing openLeveragePosition...");
                
                const tx = await leveragePlugin.openLeveragePosition(params);
                ...
                const positionCount = await leveragePlugin.getActivePositionCount();
                console.log(`   Active positions: ${positionCount}`);
                
                if (positionCount > 0n) {
                    const position = await leveragePlugin.getPosition(0);
                    console.log(`\n📊 Position Details:`);
