...
contract EnhancedLiquidityPoolETH is ERC20, ReentrancyGuard, Ownable {
    ...
    struct WithdrawValidation {
        ...
        /**
     * @notice Struct to hold token value and percentage information
     * @param value The token's value in ETH terms
     * @param percentage The token's percentage of the total pool value
     */
    struct TokenValueInfo {
        uint256 value;
        uint256 percentage; // Percentage with 2 decimal places (e.g., 1234 = 12.34%)
    }

        /**
     * @notice Struct to hold pool value information
     * @param totalValue The total pool value in ETH terms
     * @param tokenValues Array of token values and percentages
     */
    ...
    returns (uint256 ethAmount) 
{
    ...
    // Get total pool value and check if we need to perform a swap
    PoolValueInfo memory poolInfo;
    try this.getTotalPoolValue() returns (PoolValueInfo memory info) {
        poolInfo = info;
        validation.totalValue = info.totalValue;
    ...
    // If we don't have enough WETH, perform a swap
    if (validation.poolEthBalance < validation.ethAmount) {
        uint256 wethNeeded = validation.ethAmount - validation.poolEthBalance;
        
        // Find token with lowest percentage to swap
        string memory tokenToSwap;
        uint256 lowestPercentage = type(uint256).max;
        uint256 tokenAmountToSwap;
        
        // Skip index 0 as it's WETH
        for (uint i = 1; i < poolInfo.tokenValues.length; i++) {
            if (tokenData[tokenCodes[i - 1]].isActive && 
                ...
                (uint256 cachedValue, bool isCacheValid) = getCachedTokenValue(tokenToSwap);
                
                if (isCacheValid) {
                    // Use cached price
                    uint256 wethWithBuffer = (wethNeeded * 110) / 100;
                    tokenAmountToSwap = (wethWithBuffer * (10 ** tokenData[tokenToSwap].tokenDecimals)) / cachedValue;
                } else {
                    // If no valid cache, try to get current price
                    try this.getTokenPriceWithEvents(tokenToSwap) returns (uint256 price, uint256 timestamp) {
                        uint256 wethWithBuffer = (wethNeeded * 110) / 100;
                        tokenAmountToSwap = (wethWithBuffer * (10 ** tokenData[tokenToSwap].tokenDecimals)) / price;
                    } catch {
        ...
        require(tokenAmountToSwap > 0, "Invalid swap amount calculated");

        // Get token address
        address tokenAddress = tokenData[tokenToSwap].tokenAddress;

        // Approve SimpleSwap to spend our tokens if needed
        IERC20(tokenAddress).approve(simpleSwapAddress, tokenAmountToSwap);

        // Perform the swap using inputSwap
        uint256 receivedWeth = ISimpleSwap(simpleSwapAddress).inputSwap(
            tokenAddress,      // spend token address
            WETH_ADDRESS,     // receive token address
            tokenAmountToSwap // amount to swap
        );

        // Verify we now have enough WETH
        require(
            IWETH(WETH_ADDRESS).balanceOf(address(this)) >= validation.ethAmount,
            "Swap didn't provide enough WETH"
        );
    }

    // Rest of the function remains the same...
    // Check post-withdrawal reserve ratio
    uint256 postWithdrawBalance = IWETH(WETH_ADDRESS).balanceOf(address(this)) - validation.ethAmount;
    uint256 postWithdrawValue = validation.totalValue - validation.ethAmount;
    uint256 postWithdrawRatio = (postWithdrawBalance * 10000) / postWithdrawValue;
...
/**
 * @notice Calculates the total value of all assets in the pool and their percentages
 * @return PoolValueInfo containing total value and individual token percentages
 */
function getTotalPoolValue() external returns (PoolValueInfo memory) {
    // First calculate total value
    ...
    // Create array to store token values and percentages
    TokenValueInfo[] memory tokenValues = new TokenValueInfo[](tokenCodes.length + 1);
    
    // Calculate WETH percentage first
    ...
    // Second pass: calculate percentages for each token
    for (uint i = 0; i < tokenCodes.length; i++) {
        if (tokenData[tokenCodes[i]].isActive) {
...
/**
 * @notice Updates a parameter instantly
